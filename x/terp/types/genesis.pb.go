...
// GenesisState defines the terp module's genesis state.
type GenesisState struct {
	Params           Params        `protobuf:"bytes,1,opt,name=params,proto3" json:"params"`
	TerpidList   []Terpid  `protobuf:"bytes,2,rep,name=terpidList,proto3" json:"terpidList"`
	TerpidCount  uint64        `protobuf:"varint,3,opt,name=terpidCount,proto3" json:"terpidCount,omitempty"`
...
func (m *GenesisState) GetTerpidList() []Terpid {
	if m != nil {
		return m.TerpidList
	}
	return nil
...
func (m *GenesisState) GetTerpidCount() uint64 {
	if m != nil {
...
func init() {
	proto.RegisterType((*GenesisState)(nil), "terpnetwork.terp.terp.GenesisState")
}

func init() { proto.RegisterFile("terp/genesis.proto", fileDescriptor_a0474e07bc9bd608) }
...
func (m *GenesisState) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.TerpidCount != 0 {
		i = encodeVarintGenesis(dAtA, i, uint64(m.TerpidCount))
		i--
		dAtA[i] = 0x18
	}
	if len(m.TerpidList) > 0 {
		for iNdEx := len(m.TerpidList) - 1; iNdEx >= 0; iNdEx-- {
			{
				size, err := m.TerpidList[iNdEx].MarshalToSizedBuffer(dAtA[:i])
				if err != nil {
					return 0, err
				}
				i -= size
...
func (m *GenesisState) Size() (n int) {
	...
	if len(m.TerpidList) > 0 {
		for _, e := range m.TerpidList {
			l = e.Size()
			n += 1 + l + sovGenesis(uint64(l))
		}
	}
	if m.TerpidCount != 0 {
		n += 1 + sovGenesis(uint64(m.TerpidCount))
	}
	if len(m.SupplychainList) > 0 {
...
func (m *GenesisState) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TerpidList", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.TerpidList = append(m.TerpidList, Terpid{})
			if err := m.TerpidList[len(m.TerpidList)-1].Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 3:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field TerpidCount", wireType)
			}
			m.TerpidCount = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.TerpidCount |= uint64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
