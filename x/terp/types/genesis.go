...
func (gs GenesisState) Validate() error {
	// Check for duplicated ID in terpid
	terpidIdMap := make(map[uint64]bool)
	terpidCount := gs.GetTerpidCount()
	for _, elem := range gs.TerpidList {
		if _, ok := terpidIdMap[elem.Id]; ok {
			return fmt.Errorf("duplicated id for terpid")
		}
		if elem.Id >= terpidCount {
			return fmt.Errorf("terpid id should be lower or equal than the last id")
		}
		terpidIdMap[elem.Id] = true
	}
