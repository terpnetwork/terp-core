...
import (
	...
	terputiils "github.com/terpnetwork/terp-core/terputils"
...
func (k Keeper) BeginBlocker(ctx sdk.Context) {
	...
	k.IterateEpochInfo(ctx, func(_ int64, epochInfo types.EpochInfo) (stop bool) {
		...
		case shouldEpochStart:
			...
			logger.Info(terputiils.LogHeader("%s EPOCH %d", epochAlias, epochInfo.CurrentEpoch))
			logger.Info(terputiils.LogHeader("Epoch Start Time: %s", epochInfo.CurrentEpochStartTime))
