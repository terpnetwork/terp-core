...
func (s *AuthenticatorPostSuite) TestAuthenticatorPostHandlerFailConfirmExecution() {
	...
	s.TerpApp.AuthenticatorManager.RegisterAuthenticator(approveAndBlock)
	approveAndBlockId, err := s.TerpApp.SmartAccountKeeper.AddAuthenticator(s.Ctx, s.TestAccAddress[0], approveAndBlock.Type(), []byte{})
