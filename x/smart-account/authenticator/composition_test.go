	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			...
			if !tc.expectInit {
				...
				s.Require().NoError(err)
				request, err := authenticator.GenerateAuthenticationRequest(s.Ctx, s.TerpApp.AppCodec(), ak, sigModeHandler, s.TestAccAddress[0], s.TestAccAddress[0], nil, sdk.NewCoins(), msg, tx, 0, false, authenticator.SequenceMatch)
				s.Require().NoError(err)
