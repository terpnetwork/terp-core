...
func (s *CosmwasmAuthenticatorTest) SetupTest() {
	...
	s.CosmwasmAuth = authenticator.NewCosmwasmAuthenticator(s.TerpApp.ContractKeeper, s.TerpApp.AccountKeeper, s.TerpApp.AppCodec())
}
...
func (s *CosmwasmAuthenticatorTest) TestOnAuthenticatorAdded() {
	...
	// create new account
	acc := testutils.CreateRandomAccounts(1)[0]
...
func (s *CosmwasmAuthenticatorTest) TestGeneral() {
	accounts := testutils.CreateRandomAccounts(2)
	for _, acc := range accounts {
		someCoins := sdk.NewCoins(sdk.NewInt64Coin(sdk.DefaultBondDenom, 1000000))
		err := s.TerpApp.BankKeeper.MintCoins(s.Ctx, minttypes.ModuleName, someCoins)
		s.Require().NoError(err)
		err = s.TerpApp.BankKeeper.SendCoinsFromModuleToAccount(s.Ctx, minttypes.ModuleName, acc, someCoins)
		s.Require().NoError(err)
	}
	...
	ak := s.TerpApp.AccountKeeper
	sigModeHandler := s.EncodingConfig.TxConfig.SignModeHandler()
	request, err := authenticator.GenerateAuthenticationRequest(s.Ctx, s.TerpApp.AppCodec(), ak, sigModeHandler, accounts[0], accounts[0], nil, feeCoins, testMsg, tx, 0, false, authenticator.SequenceMatch)
	s.Require().NoError(err)
...
func (s *CosmwasmAuthenticatorTest) TestCosignerContract() {
	...
	ak := s.TerpApp.AccountKeeper
	sigModeHandler := s.EncodingConfig.TxConfig.SignModeHandler()
	request, err := authenticator.GenerateAuthenticationRequest(s.Ctx, s.TerpApp.AppCodec(), ak, sigModeHandler, accounts[0], accounts[0], nil, sdk.NewCoins(), testMsg, tx, 0, false, authenticator.SequenceMatch)
	s.Require().NoError(err)
