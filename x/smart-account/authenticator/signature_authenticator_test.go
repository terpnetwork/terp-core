...
func (s *SigVerifyAuthenticationSuite) TestSignatureAuthenticator() {
	...
	testMsg3 := &banktypes.MsgSend{
		FromAddress: sdk.MustBech32ifyAddressBytes(terpToken, s.TestAccAddress[2]),
		ToAddress:   sdk.MustBech32ifyAddressBytes(terpToken, s.TestAccAddress[1]),
		Amount:      coins,
	}
	...
	for _, tc := range tests {
		s.Run(tc.Description, func() {
			...
			if tc.TestData.ShouldSucceedGettingData {
				...
				request, err := authenticator.GenerateAuthenticationRequest(s.Ctx, s.TerpApp.AppCodec(), ak, sigModeHandler, addr, addr, nil, sdk.NewCoins(), tc.TestData.Msgs[0], tx, 0, false, authenticator.SequenceMatch)
				...
				_, err := authenticator.GenerateAuthenticationRequest(s.Ctx, s.TerpApp.AppCodec(), ak, sigModeHandler, addr, addr, nil, sdk.NewCoins(), tc.TestData.Msgs[0], tx, 0, false, authenticator.SequenceMatch)
				s.Require().Error(err)
			}
		})
	}
...
//	s.TerpApp.AccountKeeper.SetAccount(s.Ctx, account)
//
//	coins := sdk.Coins{sdk.NewInt64Coin(terpToken, 2500)}
