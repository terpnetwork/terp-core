...
func BuildTree(size int, cacheSize int) (itree *MutableTree, keys [][]byte, err error) {
	// ICS23 IavlSpec is SHA-256 only. Default IAVL hashing is BLAKE3.
	tree := NewMutableTree(dbm.NewMemDB(), cacheSize, false, NewNopLogger(), SHA256Option())
...
func BenchmarkConvertLeafOp(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for _, version := range versions {
			sink = convertLeafOp(version, ics23.HashOp_SHA256)
		}
	}
	if sink == nil {
