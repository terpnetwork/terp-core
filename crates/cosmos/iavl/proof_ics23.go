...
func (t *ImmutableTree) createExistenceProof(key []byte) (*ics23.ExistenceProof, error) {
	...
	hop := treeIcs23HashOp(t)
	return &ics23.ExistenceProof{
		Key:   node.key,
		Value: node.value,
		Leaf:  convertLeafOp(nodeVersion, hop),
		Path:  convertInnerOps(path, hop),
	}, err
}

func treeIcs23HashOp(t *ImmutableTree) ics23.HashOp {
	if t != nil && t.ndb != nil && t.ndb.opts.Hasher != nil {
		if t.ndb.opts.Hasher.Algorithm() == hash.BLAKE3 {
			return ics23.HashOp_BLAKE3
		}
	}
	return ics23.HashOp_SHA256
}

func convertLeafOp(version int64, hop ics23.HashOp) *ics23.LeafOp {
...
func convertInnerOps(path PathToLeaf, hop ics23.HashOp) []*ics23.InnerOp {
