...
type decoratedKeeper interface {
	...
	create_with_circuit(ctx context.Context, creator sdk.AccAddress, wasmCode, vkCode []byte, instantiateAccess *types.AccessConfig, authZ types.AuthorizationPolicy) (codeID, zkid uint64, checksums [][]byte, err error)
	instantiate(
		ctx context.Context,
		codeID uint64,
