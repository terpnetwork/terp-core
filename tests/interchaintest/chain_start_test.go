...
// TestBasicBtsgStart is a basic test to assert that spinning up a Terp network with one validator works properly.
func TestBasicTerpStart(t *testing.T) {
	...
	users := interchaintest.GetAndFundTestUsers(t, ctx, t.Name(), userFunds, terp, terp, terp)
	println("users", users)
	// chainUser := users[0]
	...
	superAdmin, err := interchaintest.GetAndFundTestUserWithMnemonic(ctx, "acc0", mnemonic, genesisAmt, terp)
	require.NoError(t, err)

	t.Run("authz", func(t *testing.T) {
		users := interchaintest.GetAndFundTestUsers(t, ctx, "default", genesisAmt, terp, terp, terp)
		testAuthz(ctx, t, terp, users)
	})

	t.Run("bank", func(t *testing.T) {
		users := interchaintest.GetAndFundTestUsers(t, ctx, "default", genesisAmt, terp, terp, terp)
		testBank(ctx, t, terp, users)
	})

	t.Run("distribution", func(t *testing.T) {
		users := interchaintest.GetAndFundTestUsers(t, ctx, "default", genesisAmt, terp, terp, terp)
		testDistribution(ctx, t, terp, users)
	})

	t.Run("feegrant", func(t *testing.T) {
		users := interchaintest.GetAndFundTestUsers(t, ctx, "default", genesisAmt, terp, terp, terp, terp)
		testFeeGrant(ctx, t, terp, users)
	})

	t.Run("gov", func(t *testing.T) {
		// users := interchaintest.GetAndFundTestUsers(t, ctx, "default", genesisAmt, terp, terp, terp)
		// testGov(ctx, t, terp, users)
	})

	t.Run("auth-vesting", func(t *testing.T) {
		testAuth(ctx, t, terp)
		testVesting(ctx, t, terp, superAdmin)
	})

	// t.Run("upgrade", func(t *testing.T) {
	// 	// testUpgrade(ctx, t, terp)
	// })

	t.Run("staking", func(t *testing.T) {
		users := interchaintest.GetAndFundTestUsers(t, ctx, "default", genesisAmt, terp, terp, terp)
		testStaking(ctx, t, terp, users)
	})

	t.Run("slashing", func(t *testing.T) {
		testSlashing(ctx, t, terp)
	})

	// t.Run("smart-account", func(t *testing.T) {
	// 	testSmartAccount(ctx, t, terp, users)
	// })
...
func testDistribution(ctx context.Context, t *testing.T, chain *cosmos.CosmosChain, users []ibc.Wallet) {
	...
	newWithdrawAddr := "terp15llesx99x6upyqlhdtxz9f6grf6rvnf7jtz7jr"
