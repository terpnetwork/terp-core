...
echo "Importing keys"
hermes keys add --key-name relayer_key_test_1 --chain "test-1" --hd-path "m/44'/118'/0'/0/0" --mnemonic-file <(jq -r '.mnemonic' ./data/test-1/relayer_seed.json)
hermes keys add --key-name relayer_key_test_2 --chain "test-2" --hd-path "m/44'/118'/0'/0/0" --mnemonic-file <(jq -r '.mnemonic' ./data/test-2/relayer_seed.json)
# hermes keys add --key-name relayer_key_test_3 --chain "test-3" --hd-path "m/44'/118'/0'/0/0" --mnemonic-file <(jq -r '.mnemonic' ./data/test-3/relayer_seed.json)
#sleep 2
