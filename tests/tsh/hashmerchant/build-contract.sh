...
CONTRACT_DIR="$SCRIPT_DIR/../../artifacts/hashmerchant-test"
...
if command -v docker &>/dev/null; then
    docker run --rm -v "$CONTRACT_DIR":/code \
        ...
        cosmwasm/optimizer:0.18.0
    cp "$CONTRACT_DIR/artifacts/hashmerchant_test.wasm" "$SCRIPT_DIR/../../artifacts/"
    echo "Optimized WASM copied to ../../artifacts/hashmerchant_test.wasm"
else
    ...
    cp "$CONTRACT_DIR/target/wasm32-unknown-unknown/release/hashmerchant_test.wasm" "$SCRIPT_DIR/../../artifacts/"
    echo "WASM copied to ../../artifacts/hashmerchant_test.wasm (not optimized)"
fi
