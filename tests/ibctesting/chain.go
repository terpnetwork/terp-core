...
func (chain *TestChain) GetTerpApp() *app.TerpApp {
	v, _ := chain.App.(*app.TerpApp)
	return v
}
...
func (chain *TestChain) SendMsgsFromPrivKeys(privKeys []cryptotypes.PrivKey, msgs ...sdk.Msg) (*abci.ExecTxResult, error) {
	...
	for i, msg := range msgs {
		...
		account := chain.GetTerpApp().AppKeepers.AccountKeeper.GetAccount(chain.GetContext(), signerAcc)
	...
	for _, msg := range msgs {
		...
		signer := signers[0]
		account := chain.GetTerpApp().AppKeepers.AccountKeeper.GetAccount(chain.GetContext(), signer)
