...
pfm=$(go list -fprot '{{ .Dir }}' -m github.com/strangelove-ventures/packet-forward-middleware/v4)
...
# delete cosmos/mint path since terp uses its own module
...
for f in $files; do
  ...
  if [[ "$f" =~ "gaia" ]]; then
    ...
    cp $f ./tmp-swagger-gen/_all/terp-$counter.json
  elif [[ "$f" =~ "cosmos" ]]; then
