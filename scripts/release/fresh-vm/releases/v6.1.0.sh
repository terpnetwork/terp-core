# Historical extras for tag v6.1.0. Sourced by fresh-vm run.sh / guest.sh.
# ibc-hooks-v11 is gitignored — guest fetches the public tarball (checksummed).
# Hasher vendor is NOT on this tag (see networks/upgrades/v6.1/HASHER.md).
...
fresh_vm_host_stage() {
  local extras="$1"
  local lock="${HOST_ROOT:-}/networks/upgrades/v6.1/ARTIFACT_LOCK"
  if [ -f "$lock" ]; then
...
fresh_vm_verify() {
  if [ -d crates/cosmos/iavl ] || [ -d crates/cosmos/store-v2 ]; then
    echo "ERROR: hasher vendor present on fresh v6.1.0 clone (must not ship in the tag)" >&2
    fail=1
  fi
  if grep -q 'replace github.com/cosmos/cosmos-sdk/store/v2 => ./crates/cosmos/store-v2' go.mod; then
    ...
    echo "NOTE: v6.1.0 does not replace store/v2; live CMS hasher is stock SHA-256"
  fi
