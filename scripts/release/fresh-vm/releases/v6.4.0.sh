...
# Same 4.0.0-zk muslc as v6.3.0 (2026-09-21 recut, Path A DSTW reject).
...
fresh_vm_verify() {
  if ! grep -q 'github.com/cosmos/iavl => ./crates/cosmos/iavl' go.mod; then
    echo "ERROR: v6.4.0 tag missing iavl replace — hasher will not compile" >&2
    fail=1
  fi
  if ! grep -q 'github.com/cosmos/cosmos-sdk/store/v2 => ./crates/cosmos/store-v2' go.mod; then
    echo "ERROR: v6.4.0 tag missing store/v2 replace" >&2
    fail=1
  fi
}
