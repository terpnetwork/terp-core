#!/bin/bash
# Run this script to quickly install, setup, and run the current version of terp without docker.
#
# Example:
# CHAIN_ID="local-1" HOME_DIR="~/.terp1" TIMEOUT_COMMIT="500ms" CLEAN=true sh scripts/test_node.sh
# CHAIN_ID="local-2" HOME_DIR="~/.terp2" CLEAN=true RPC=36657 REST=2317 PROFF=6061 P2P=36656 GRPC=8090 GRPC_WEB=8091 ROSETTA=8081 TIMEOUT_COMMIT="500ms" sh scripts/test_node.sh
#
...
from_scratch () {
  ...
  # Run this to ensure terprything worked and that the genesis file is setup correctly
...
BINARY start --pruning=nothing  --minimum-gas-prices=0uterpx --rpc.laddr="tcp://0.0.0.0:$RPC" --wasm.skip_wasmvm_version_check --log_level trace