...
Public validator guides: [`v6.1/guide.md`](./v6.1/guide.md) · [`v6.2/guide.md`](./v6.2/guide.md).

**Status:** Standard (not expedited) plan **`v6.1` at height `23191300`**. Voting period is **96 hours**. Halt is **12 hours after** voting ends (~**108 hours** from 2026-09-08 06:54 UTC). Height uses **3.0 s/block** so 3.0–3.5 s cannot halt during the vote (live mean **3.53 s**, not 2.4 s). Between now and height `23191300`, every validator and sentry must run **`cosmovisor run start`**, not raw `terpd start`. Plan **`v6.2` is not a second proposal** — the v6.1 binary arms it at apply + 2.
...
DAEMON_NAME=terpd DAEMON_RESTART_AFTER_UPGRADE=true. No darwin. Plan v6.1 halt height 23191300 (96h vote + 12h; sized at 3.0 s/block, not 2.4 s). v6.2 is apply+2 from the v6.1 binary.
```
